...
Grapic::Grapic()
    : m_width(-1), m_height(-1), m_window(nullptr), m_renderer(nullptr),
      m_font(nullptr), m_fontFile(""), m_fontSize(-1), m_quit(false),
      m_anim(false), m_keyRepeatMode(false), imagesSavedCount(0) {
  m_currentColor.r = 0;
  m_currentColor.g = 0;
  m_currentColor.b = 0;
...
void Grapic::init(const char *name, int w, int h, int posx, int posy,
                  SDL_WindowFlags flag, std::string render_driver) {
  ...
  if (TTF_Init() != 0) {
    std::cout << "Erreur lors de l'initialisation de la SDL_ttf : "
              << SDL_GetError() << std::endl;
    SDL_Quit();
    assert(0);
    exit(1);
  ...
  m_window =
      SDL_CreateWindow(name, posx, posy, w, h,
                       SDL_WINDOW_SHOWN | flag); //| SDL_WINDOW_RESIZABLE);
  if (m_window == nullptr) {
  ...
  if (m_renderInfo.flags & SDL_RENDERER_SOFTWARE) {
    cout << "  SDL_RENDERER_SOFTWARE ";
  }
  if (m_renderInfo.flags & SDL_RENDERER_ACCELERATED) {
    cout << "  SDL_RENDERER_ACCELERATED ";
  }
  if (m_renderInfo.flags & SDL_RENDERER_PRESENTVSYNC) {
    cout << "  SDL_RENDERER_PRESENTVSYNC ";
  }
  if (m_renderInfo.flags & SDL_RENDERER_TARGETTEXTURE) {
    cout << "  SDL_RENDERER_TARGETTEXTURE ";
  }
  cout << endl;
  ...
  cout << endl;
  cout << "   texture format default: "
       << SDL_GetPixelFormatName(m_textureFormat) << "(" << m_textureFormat
       << ")" << endl;

  cout << "   Max Texture Width: " << m_renderInfo.max_texture_width
       << std::endl;
  cout << "   Max Texture Height: " << m_renderInfo.max_texture_height
       << std::endl;
  cout << "----------" << endl;
...
const SDL_Surface *Image::surface() const { return m_surface; }
...
void Menu::change(int i, const std::string &str) {
  if ((i >= 0) && (i < m_txt.size()))
    ...
    std::cerr << "menu_change(...): i is not in the range of the menu"
              << std::endl;
}
...
int Menu::select() const { return m_select; }
...
void Menu::add(const std::string &str) { m_txt.push_back(str); }

bool saveScreenshotPNG(std::string filepath, SDL_Window *SDLWindow,
  ...
  if (infoSurface == nullptr) {
    ...
    if (pixels == 0) {
      ...
      if (SDL_RenderReadPixels(SDLRenderer, &infoSurface->clip_rect,
                               ...
                               infoSurface->w *
                                   infoSurface->format->BytesPerPixel) != 0) {
...
bool Grapic::manageOneEvent(SDL_Event event) {
  ...
  case SDL_KEYDOWN:
    if (((size_t)event.key.keysym.scancode < m_keyStates.size()) &&
        ((m_keyRepeatMode) || (event.key.repeat == 0))) {
...
Grapic &Grapic::singleton(bool secure) {
  if (secure) {
    if (!currentGrapic->isInit()) {
      std::cout
          << "You have to call winInit before any call to Grapic functions !"
          << std::endl;
      exit(1);
    }
  }
...
void Grapic::clear() {
  ...
  SDL_SetRenderDrawColor(m_renderer, m_currentColor.r, m_currentColor.g,
...
void Grapic::setKeyRepeatMode(bool kr) { m_keyRepeatMode = kr; }

void Grapic::color(unsigned char r, unsigned char g, unsigned char b,
                   unsigned char a) {
  m_currentColor.r = r;
  m_currentColor.g = g;
  m_currentColor.b = b;
...
SDL_Color &Grapic::getColor() { return m_currentColor; }

SDL_Color &Grapic::getBackgroundColor() { return m_backgroundColor; }

void Grapic::backgroundColor(unsigned char r, unsigned char g, unsigned char b,
                             unsigned char a) {
...
void Grapic::setFont(int s, const char *ttf) {
  if ((m_font) && (m_fontSize == s) && (ttf && m_fontFile == std::string(ttf)))
    return;
  if (m_font)
    TTF_CloseFont(m_font);
  m_fontSize = s;
  ...
  if (!m_font)
    m_font = TTF_OpenFont(("../../../../" + m_fontFile).c_str(), m_fontSize);
  if (!m_font) {
    std::cout << "Erreur lors de l'initialisation de la police : "
              << SDL_GetError() << std::endl;
    SDL_Quit();
    assert(0);
    exit(1);
...
inline SDL_Renderer *renderer() { return Grapic::singleton().renderer(); }

void winSetPosition(int w, int h, int px, int py, bool fullscreen) {
  if (fullscreen)
    SDL_SetWindowFullscreen(Grapic::singleton().window(),
                            SDL_WINDOW_FULLSCREEN);
  else
    SDL_SetWindowFullscreen(Grapic::singleton().window(), 0);
...
int filledEllipseRGBA(SDL_Renderer *m_renderer, Sint16 x, Sint16 y, Sint16 rx,
  ...
  result |= SDL_SetRenderDrawBlendMode(
      m_renderer, (a == 255) ? SDL_BLENDMODE_NONE : SDL_BLENDMODE_BLEND);
  ...
  if (rx > ry) {
    ...
    do {
      ...
      if ((ok != k) && (oj != k)) {
        ...
        if (k > 0) {
          result |=
              SDL_RenderDrawLine(m_renderer, xmh, y + k, xph,
                                 y + k); // hline(renderer, xmh, xph, y + k);
          result |=
              SDL_RenderDrawLine(m_renderer, xmh, y - k, xph,
                                 y - k); // hline(renderer, xmh, xph, y - k);
        } else {
          result |= SDL_RenderDrawLine(m_renderer, xmh, y, xph,
                                       y); // hline(renderer, xmh, xph, y);
        }
        ok = k;
      }
      if ((oj != j) && (ok != j) && (k != j)) {
        ...
        if (j > 0) {
          result |=
              SDL_RenderDrawLine(m_renderer, xmi, y + j, xpi,
                                 y + j); // hline(renderer, xmi, xpi, y + j);
          result |=
              SDL_RenderDrawLine(m_renderer, xmi, y - j, xpi,
                                 y - j); // hline(renderer, xmi, xpi, y - j);
        } else {
          result |= SDL_RenderDrawLine(m_renderer, xmi, y, xpi,
                                       y); // hline(renderer, xmi, xpi, y);
        }
        oj = j;
      }
    ...
    do {
      ...
      if ((oi != i) && (oh != i)) {
        ...
        if (i > 0) {
          result |=
              SDL_RenderDrawLine(m_renderer, xmj, y + i, xpj,
                                 y + i); // hline(renderer, xmj, xpj, y + i);
          result |=
              SDL_RenderDrawLine(m_renderer, xmj, y - i, xpj,
                                 y - i); // hline(renderer, xmj, xpj, y - i);
        } else {
          result |= SDL_RenderDrawLine(m_renderer, xmj, y, xpj,
                                       y); // hline(renderer, xmj, xpj, y);
        }
        oi = i;
      }
      if ((oh != h) && (oi != h) && (i != h)) {
        ...
        if (h > 0) {
          result |=
              SDL_RenderDrawLine(m_renderer, xmk, y + h, xpk,
                                 y + h); // hline(renderer, xmk, xpk, y + h);
          result |=
              SDL_RenderDrawLine(m_renderer, xmk, y - h, xpk,
                                 y - h); // hline(renderer, xmk, xpk, y - h);
        } else {
          result |= SDL_RenderDrawLine(m_renderer, xmk, y, xpk,
                                       y); // hline(renderer, xmk, xpk, y);
        }
        oh = h;
      }
...
void circle(int xc, int yc, int circleR) {
  ...
  Uint8 *c = (Uint8 *)&cc;
  aaellipseRGBA(g.renderer(), xc, g.inverseY(yc), circleR, circleR, c[0], c[1],
                c[2], c[3]);
}
...
void circleFill(int xc, int yc, int circleR) {
  ...
  Uint32 cc = *((Uint32 *)(&g.getColor()));
  Uint8 *c = (Uint8 *)&cc;
  filledEllipseRGBA(g.renderer(), xc, g.inverseY(yc), circleR, circleR, c[0],
                    c[1], c[2], c[3]);
}
...
void print(int x, int y, const char *txt) {
  ...
  SDL_Rect renderQuad = {x, g.inverseY(y + text_height), text_width,
                         text_height};
...
Uint32 image_get(SDL_Surface *surface, int x, int y) {
  ...
  y = surface->h - 1 - y;
  Uint8 *p =
      (Uint8 *)surface->pixels + y * surface->pitch + x * nbOctetsParPixel;
...
Image::Image()
    : m_surface(nullptr), m_texture(nullptr), m_has_changed(nullptr),
      nb_instance(nullptr) {
...
void Image::init(int w, int h) {
  ...
  int bpp = 32;
  SDL_PixelFormatEnumToMasks(g.textureFormat(), &bpp, &rmask, &gmask, &bmask,
                             &amask);
  m_surface = SDL_CreateRGBSurface(0, w, h, 32, rmask, gmask, bmask,
                                   amask); // 0, 0, 0, 0);
...
Image::Image(const char *filename, bool transparency, unsigned char r,
  ...
  SDL_Surface *surfaceCorrectPixelFormat = SDL_ConvertSurfaceFormat(
      m_surface, gp.textureFormat(), 0); // SDL_PIXELFORMAT_RGBA8888,0);
...
void Image::set(int x, int y, unsigned char r, unsigned char g, unsigned b,
                unsigned char a) {
  assert(m_surface);
...
void Image::draw(int x, int y, int w, int h) {
  ...
  if (*m_has_changed) {
    ok = SDL_UpdateTexture(m_texture, nullptr, m_surface->pixels,
                           m_surface->pitch);
    assert(ok == 0);
    *m_has_changed = false;
  }
...
void Image::draw(int x, int y, int w, int h, float angle, int flip) {
  ...
  ok = SDL_RenderCopyEx(g.renderer(), m_texture, nullptr, &r, angle, nullptr,
                        fl);
  assert(ok == 0);
}
...
void Image::savePNG(const char *filename) const {
  ...
  SDL_Surface *surfaceCorrectPixelFormat =
      SDL_ConvertSurfaceFormat(m_surface, g.textureFormat(), 0);
  int r = IMG_SavePNG(surfaceCorrectPixelFormat, filename);
  if (r != 0)
    printf("image: error saving %s\n", filename);
  else
    printf("Image saved: %s   %dx%d format=%d %s pitch=%d\n", filename,
           surfaceCorrectPixelFormat->w, surfaceCorrectPixelFormat->h,
...
void Image::printInfo() const {
  ...
  printf("imageInfo: %dx%d format=%d %s pitch=%d", m_surface->w, m_surface->h,
         m_surface->format->format,
         SDL_GetPixelFormatName(m_surface->format->format), m_surface->pitch);
  printf(" surface=%p  texture=%p has_changed=%p  nb_instance=%p", m_surface,
         m_texture, m_has_changed, nb_instance);
  if (nb_instance)
    printf(" *nb_instance=%d ", *nb_instance);
  printf("\n");
}
...
void Menu::draw(int xmin, int ymin, int xmax, int ymax) {
  ...
  for (i = 0; i < m_txt.size(); ++i) {
    if (i == m_select)
      ...
      color(255, 255, 255);
    rectangleFill(xmin, caseToPixel(i, ymin, ymax), xmax,
                  caseToPixel(i + 1, ymin, ymax));

    color(0, 0, 0); // Change the default color (the color of the pen)
    rectangle(xmin, caseToPixel(i, ymin, ymax), xmax,
              caseToPixel(i + 1, ymin, ymax));

    print(xmin + 10,
          caseToPixel(i + 1, ymin, ymax) +
              (((ymax - ymin) / m_txt.size()) - fontsize) / 2 - 3,
          m_txt[i].c_str());
  }
}
...
void Plot::add(float x, float y, int curve_n) {
  ...
  if (curve_n >= m_dat.size())
    m_dat.resize(curve_n + 1);
...
void Plot::minMax(float &fxmin, float &fymin, float &fxmax, float &fymax,
                  int &maxsize) const {
  int i, j;
  if (m_dat.size() == 0)
    return;
  ...
  for (j = 0; j < m_dat.size(); ++j) {
    ...
    for (i = 0; i < cu.size(); ++i) {
      ...
      if (cu[i].first < fxmin)
        fxmin = cu[i].first;
      if (cu[i].second > fymax)
        fymax = cu[i].second;
      if (cu[i].second < fymin)
        fymin = cu[i].second;
    }
  }
}

void Plot::draw(const Curve &cu, int xmin, int ymin, int xmax, int ymax,
...
void Plot::draw(int xmin, int ymin, int xmax, int ymax, bool clearOrNot) const {
  ...
  SDL_Color save = gr.getColor();
  const int N = 5;
  static const SDL_Color colcu[] = {{0, 255, 255, 255},
                                    {0, 255, 0, 255},
                                    {0, 0, 255, 255},
                                    {255, 255, 0, 255},
                                    {255, 0, 255, 255}};
...
void regular_polygone(int x, int y, unsigned int apotheme,
  ...
  for (unsigned int i = 0; i < line_number; i++) {
    ...
    line(x + apotheme * cos((2.f * 3.14f * i) / line_number),
         y + apotheme * sin((2.f * 3.14f * i) / line_number),
         x + apotheme * cos((2.f * 3.14f * (i + 1)) / line_number),
         y + apotheme * sin((2.f * 3.14f * (i + 1)) / line_number));
  }
}
...
#define MIN(a, b) ((a < b) ? a : b)
  ...
  for (int i = bottomY; i < topY; i++) {
    // Draw lines which are parallel to the x-axis for each pixel on the "main"
    // line delimited by (topX, topY) and (bottomX, bottomY)
...
void ellipseFill(int xc, int yc, int h, int v) {
  ...
  filledEllipseRGBA(g.renderer(), xc, g.inverseY(yc), h, v, c[0], c[1], c[2],
                    c[3]);
}
...
void regular_polygonFill(int x, int y, unsigned int apotheme,
  ...
  if (line_number == 3) {
    triangleFill(x + apotheme, y, x - apotheme * 0.5, y + apotheme * 0.866,
  ...
  for (unsigned int i = 0; i < line_number; i++) {
    triangleFill(x, y, // Point 1 (centre)
                 x + apotheme * cos((2.f * 3.14f * i) / line_number),
                 y + apotheme * sin((2.f * 3.14f * i) / line_number), // Point 2
                 x + apotheme * cos((2.f * 3.14f * (i + 1)) / line_number),
                 y + apotheme *
                         sin((2.f * 3.14f * (i + 1)) / line_number)); // Point 3
  }
}
...
void polygon(int p[][2], unsigned int number) {
  for (unsigned int i = 0; i < number; i++) {
    grapic::line(p[i % number][0], p[i % number][1], p[(i + 1) % number][0],
                 p[(i + 1) % number][1]);
  }
}
