...
public:
  ...
  void init(const char *name, int w, int h, int posx = -1, int posy = -1,
            SDL_WindowFlags flag = SDL_WindowFlags(0),
  ...
  void backgroundColor(unsigned char r, unsigned char g, unsigned char b,
                       unsigned char a = 255);
...
public:
  Image();
  Image(const char *filename, bool transparency, unsigned char r,
        unsigned char g, unsigned b, unsigned char a);
  ...
  void set(int x, int y, unsigned char r, unsigned char g, unsigned b,
           unsigned char a);
...
public:
  ...
  void draw(const Curve &cu, int xmin, int ymin, int xmax, int ymax,
            float fxmin, float fymin, float fxmax, float fymax) const;
  void minMax(float &fxmin, float &fymin, float &fxmax, float &fymax,
              int &maxsize) const;
...
*/
inline void winInit(const char *name, int w, int h, int posx = -1,
                    int posy = -1) {
...
/** \brief Clear the window with the default background color
    ...
    ~~~~~~~~~~~~~~~
*/
inline void winClear() { grapic::Grapic::singleton().clear(); }

/** \brief return true if the application should be closed (because of 'ESC' or
 * 'q' key pressed for instance)
 */
inline bool winHasFinished() { return Grapic::singleton().hasFinished(); }

/** \brief Clear the event queue of the window
 */
inline void winClearEvent() { grapic::Grapic::singleton().clearEvent(); }
...
/** \brief Display the window. All drawing is hidden until this function is not
   called.
    ~~~~~~~~~~~~~~~{.c}
        bool stop=false;
        winInit("Tutorials", 500, 500);
    ...
    ~~~~~~~~~~~~~~~
*/
inline bool winDisplay() { return Grapic::singleton().display(); }
 ...
 */
inline void color(unsigned char _r = 255, unsigned char _g = 255,
 ...
 */
inline void colorf(float _r = 1.f, float _g = 1.f, float _b = 1.f,
                   float _a = 1.f) {
  Grapic::singleton().colorf(_r, _g, _b, _a);
}

/** \brief Change the default background color (the color used to clear the
 * screen)
 */
inline void backgroundColor(unsigned char _r = 255, unsigned char _g = 255,
                            unsigned char _b = 255, unsigned char _a = 255) {
 ...
 * screen)
 */
inline void backgroundColorf(float _r = 1.f, float _g = 1.f, float _b = 1.f,
                             float _a = 1.f) {
...
/** \brief Draw an ellipse at (xc, yc), horizontal radius and vertical radius,
 * rotation optional (Thanks to Anass LAHNIN)
 */
void ellipse(int xc, int yc, int horizontal, int vertical);

/** \brief Draw a filled ellipse at (xc, yc), horizontal radius and vertical
 * radius, rotation optional  (a bit long when drawing at angle != {90*k, k∈ℤ})
 */
...
/** \brief Draw a grid from (xmin,ymin) to (xmax,ymax) with nx columns and ny
 * rows
 */
 ...
 */
inline float elapsedTime() { return 0.001f * SDL_GetTicks(); }

/** \brief return the number of time the key 'key' has been pressed since the
   last call to this function.
    ~~~~~~~~~~~~~~~{.c}
        if (isKeyPressed(SDLK_LEFT)) { ... }     // test if the left arrow is
   pressed if (isKeyPressed(SDLK_LEFT)) {  ... }    // if the left arrow is
   pressed then do ... if (isKeyPressed(SDLK_a)) {  ... }       // if the key
   'a' is pressed then do ... if (isKeyPressed('a')) { ... }          // if the
   key 'a' is pressed then do ...
    ~~~~~~~~~~~~~~~
*/
...
/** \brief (de)Activate the repeat mode: when the user presses continuously on
 * the key touch is repeated. It has to be set at the beguinning of the program.
 */
...
/** \brief Stop the program during d milliseconds
    ~~~~~~~~~~~~~~~{.c}
        if (isKeyPressed(SDLK_LEFT)) {  ... }    // if the left arrow is pressed
   then do ... if (isKeyPressed(SDLK_a)) {  ... }       // if the key 'a' is
   pressed then do ... if (isKeyPressed('a')) { ... }          // if the key 'a'
   is pressed then do ...
    ~~~~~~~~~~~~~~~
*/
...
/** \brief return true if the mouse button 'button' is pressed
    ~~~~~~~~~~~~~~~{.c}
    if (isMousePressed(SDL_BUTTON_LEFT)) { ... }    // test if the left button
...
/** \brief After this function (x,y) store the mouse position
    ~~~~~~~~~~~~~~~{.c}
    int x,y;
    mousePos(x,y); // test if the left button of the mouse is pressed if
   ((isMousePressed(SDL_BUTTON_LEFT)) && (x>10) && (y>10) && (x<50) && (y<50)) {
   ... }  // if the user clicks if the square (10,10)(50,50)
    ~~~~~~~~~~~~~~~
*/
...
/** \brief After this function (x,y) store the mouse position
    ~~~~~~~~~~~~~~~{.c}
    int x,y;
    mousePosGlobal(x,y); // test if the left button of the mouse is pressed if
   ((isMousePressed(SDL_BUTTON_LEFT)) && (x>10) && (y>10) && (x<50) && (y<50)) {
   ... }  // if the user clicks if the square (10,10)(50,50)
    ~~~~~~~~~~~~~~~
*/
 ...
 */
inline bool winManageEvent() { return Grapic::singleton().manageEvent(); }

/** \brief Change the default size of the font
 */
inline void fontSize(int s) { Grapic::singleton().setFont(s); }

/** \brief Print the text txt , up left corner is (x,y)
    ~~~~~~~~~~~~~~~{.c}
    fontSize(24);                               // Change the default font to
   the size 24 print( 10, 20, "Bonjour");                  // Print the text
   "Bonjour" with the left corner (10,20)
    ~~~~~~~~~~~~~~~
*/
...
/** \brief Print the integer nb, up left corner is (x,y)
    ~~~~~~~~~~~~~~~{.c}
    print( 10, 20, 128);                      // Print the number "128" with the
   left corner (10,20)
    ~~~~~~~~~~~~~~~
*/
...
/** \brief Print the float nb, up left corner is (x,y)
    ~~~~~~~~~~~~~~~{.c}
    print( 10, 20, 3.1415);                      // Print the number "3.1415"
   with the left corner (10,20)
    ~~~~~~~~~~~~~~~
*/
...
/** \brief Return an image loaded from the file filename
   ...
   (255,255,255,255)
    ...
    void draw(Data& d)
    {
        image_draw(d.im, 0, 255);                                       // Draw
   the image with the left corner (0,255)
    }
    ~~~~~~~~~~~~~~~
*/
inline Image image(const char *filename, bool transparency = false,
                   unsigned char r = 255, unsigned char g = 255,
                   unsigned b = 255, unsigned char a = 255) {
  return Image(filename, transparency, r, g, b, a);
}

/** \brief Return an image of width=w and height=h
 */
inline Image image(int w, int h) { return Image(w, h); }

/** \brief Return a copy of the image. It duplicates the image. It is useful
 * since an affectation shares the same image.
 */
...
/** \brief Draw the image at position (x,y) with width=w and height=h (if w<0 or
 * h<0 the original size of the image is used)
 */
...
/** \brief Draw the image at position (x,y) with width=w and height=h (if w<0 or
 * h<0 the original size of the image is used); angle indicate the angle of
 * rotation and flip: 0=no flip, 1=horizontal flip, 2=vertical flip
 */
inline void image_draw(Image &im, int x, int y, int w, int h, float angle,
                       float flip = SDL_FLIP_NONE) {
  im.draw(x, y, w, h, angle, flip);
}

/** \brief return the color component c of the pixel (x,y) of the image im.
    c must be 0 for the red component, 1 for the green component, 2 for the blue
   component or 3 for the alpha/opacity component.
*/
 ...
 */
void image_set(Image &im, int x, int y, unsigned char r, unsigned char g,
...
/** \brief return true if the image is initialized
    ~~~~~~~~~~~~~~~{.c}
        d.im = image("ProgramData/grapic.bmp", true, 255, 255, 255, 255);
    if (!image_isInit(d.im))                                            // if
   the image was not loaded, try an other path d.im =
   image("../ProgramData/grapic.bmp", true, 255,255,255,255 );
    ~~~~~~~~~~~~~~~
*/
inline bool image_isInit(const Image &im) { return im.isInit(); }

/** \brief Print the informations of the image im
 */
inline void image_printInfo(const Image &im) { im.printInfo(); }
...
/** \brief Add a line to the menu m with the text str
    ...
    ~~~~~~~~~~~~~~~
*/
inline void menu_add(Menu &m, const std::string &str) { m.add(str); }
...
inline void menu_draw(Menu &m, int xmin = 5, int ymin = 5, int xmax = -1,
                      int ymax = -1) {
  m.draw(xmin, ymin, xmax, ymax);
}

//! \brief return the line selected in the menu. See menu_add for an example of
//! usage.
inline int menu_select(const Menu &m) { return m.select(); }

//! \brief return the line selected in the menu. See menu_add for an example of
//! usage.
inline void menu_setSelect(Menu &m, int s) { m.setSelect(s); }
...
inline void plot_clear(Plot &p) { p.clear(); }

//! \brief Define the size of the stored value of the funtion (<0 means
//! infinity)
inline void plot_setSize(Plot &p, const int n) { p.setSize(n); }
...
//! \brief Draw the curve in the rectangle (xmin,ymin,xmax,ymax); clear the
//! rectangle if clearOrNot is true
inline void plot_draw(const Plot &p, int xmin, int ymin, int xmax, int ymax,
                      bool clearOrNot = true) {
  p.draw(xmin, ymin, xmax, ymax, clearOrNot);
}

/**
    \brief Draw a triangle from the vertices (x1, y1), (x2, y2) and (x3, y3).
   (Code provided by Bastien DOIGNIES, many thanks)
 */
void triangle(int x1, int y1, int x2, int y2, int x3, int y3);

/**
    \brief Draw a regular polygon with line_numbers edges centred on (x, y).
   (Code provided by Bastien DOIGNIES, many thanks)
 */
void regular_polygone(int x, int y, unsigned int apotheme,
...
/**
    \brief Draw a filled regular polygon with line_numbers edges centred on (x,
   y). (Code provided by Bastien DOIGNIES, many thanks)
*/
void regular_polygonFill(int x, int y, unsigned int apotheme,
                         unsigned int line_number);

/**
    \brief Decide if a point (px, py) is inside the triangle (ax, ay), (bx, by),
   (cx, xy). (Code provided by Bastien DOIGNIES, many thanks) \return true if
   inside, false otherwise
 */
bool isInTriangle(float px, float py, float ax, float ay, float bx, float by,
                  float cx, float cy);

/** \brief  Draw a simple(no edge-crossing) and without holes filled polygon
 * \param   p[]    : array of integer which represent the coordinate of each
 vertices
 * \param   number : number of element in p[], must be even (point are two
 integers each)!
 * \warning Draw nothing if there are not enough vertices or if number is not
 even
...
GrAPiC is a small, easy-to-use framework to add graphical functionalities to
your C/C++ code. Its main goal is to provide easy graphical functions to be used
in introduction courses of algorithm and programming in C/C++. It is based on
SDL2 but to use it you need to know only few very simples functions. The Grapic
archive comes with the needed SDL part, you do not have to install SDL. Just
unzip and enjoy ! <br> <br> <br>
...
Or you can clone a version from the GitHub repo :
[https://github.com/ucacaxm/grapic](https://github.com/ucacaxm/grapic)
...
\subsection win Run on Windows with Codeblocks (the default)
  - Install Codeblocks 20.03 (IMPORTANT) from:
[http://www.codeblocks.org/downloads](http://www.codeblocks.org/downloads)\n
  ...
  - Run with CodeBlocks
     ...
     - Select in the projects manager (on the left) the project you want with
the right button "Activate project": \image html cb_activatePrj.jpg "Selection
of the project in Codeblocks" width=400
     - And run the program !
\\
\\
\subsection winVSCode Run on Windows with Visual Code/VSCode (mostly for
advanced users).
  - VSCode is not Visual Studio
  - Install MinGW as described here:
     - [Using GCC with MinGW for Visual Studio Code on
...
'settings' to open your Windows Settings. \n
          - Search for Edit environment variables for your account. \n
          - Choose the Path variable and then select Edit. \n
          - Select New and add the Mingw-w64 destination folder path to the
system path. The exact path depends on which version of Mingw-w64 you have
...
Mingw-w64, then add this to the path: `C:\msys64\mingw64\bin`. \n
          - Select OK to save the updated PATH. You will need to reopen any
console windows for the new PATH location to be available \n
...
VSCode](https://marketplace.visualstudio.com/items?itemName=ms-vscode.cpptools)\\
  - Compile and Run: first option with the file tasks.json
     - Open Visual Studio Code, then menu File+Open Folder and select
grapic.code-workspace in the the grapic folder.\n
     - Compile/Build: Ctrl+Shift+B. It calls the makefile and compile the
default file which is apps/start/main_start.cpp.
     - Run/Test: Ctrl+Shift+P and enter `run test`. It runs the default
executable: bin\start.exe. You can add a shortcut.
  ...
  - Compile and Run: second option with the terminal
     ...
     - Test if the installation of MinGW and the path setting are OK: run `g++`
and `mingw32-make` just to see if ok\n
     - Change directory (and stay always in this directory): `cd
build\\windows-cb20`
     - To compile: `mingw32-make -f Start.make`
     - To run: `..\\..\\bin\\Start.exe`
  - Compile and Run: third option with CMake
     - install cmake support for Visual Code and open
...
\subsection linux Run on Linux
  - Install SDL2, SDL_Image for SDL2 and SDL_ttf for SDL2 ; for instance with
    \code
    sudo sudo apt install g++ premake4 zip doxygen libsdl2-dev libsdl2-mixer-dev
libsdl2-image-dev libsdl2-ttf-dev \endcode
...
of the project in Codeblocks" width=400
  ...
  - Compile one program and run in the Terminal:
     - cd ....grapic
     - make projectName    (replace projectName by the name of your project:
'make list' provides the list of all projects)
     - bin/projectName
...
\subsection mac Run on MacOS
  ...
  - <b>If you have a Mac M1 (ARM processor ARM, not an INTEL), you need Rosetta
2</b>
...
https://infoidevice.fr/comment-installer-rosetta-2-sur-mac-apple-silicon/
  ...
  - With XCode
    - Run a terminal this command to launch XCode, or launch XCode from the menu
      ...
      \endcode
    - Open a project in this path:
grapic-V.V.VV-macosx/build/macosx/Start.xcodeproj
    - Compile and run
  ...
  - Run in a terminal (bash)
    - Enter these commands to compile:
        \code
        cd .../grapic-V.V.VV-MacOS          # trouver le chemin vers l'endroit
où vous avez décompressé le .zip make \endcode
    - To run: `bin/Start`
...
https://repl.it/@ucax217/grapic-light#main.cpp
  ...
  - "run": compilation errors are in the terminal on the right. Execution is on
the top right.



\subsection edit Edit my first program
The kernel source code of grapic is in grapic/src. The source codes of the
programs/demos/tutorials are in grapic/apps. You can edit your first program by
editing the file grapic/apps/start/main_start.cpp \image html cb_editStart.jpg
width=10


\subsection create Create a new project
You can create your own project.
 - Copy the directory apps/start into apps/MYPROJECT  (replace MYPROJECT by the
name you want)
...
If you are following the course LIFAMI, run lifami.bat on windows or enter "make
lifami" on Linux in the command line, it will create for you all the projects
for the tutorials.
...
\image html tuto_simple.jpg "The code of the tutorial 1 "simpleDrawing"
illustrates the basis of Grapic" width=200px \include
"../apps/tutorials/tuto1_Simple.cpp"

<br>
<br>
<br>
\subsection tuto2 Tutorial 2: First Loop
\image html tuto_drawing.jpg "The code of the tutorial 2 shows how to write the
main loop of a Grapic application" width=200px \include
"../apps/tutorials/tuto2_FirstLoop.cpp"

<br>
<br>
<br>
\subsection tuto3 Tutorial 3: Drawing
\image html tuto_drawing.jpg "The code of the tutorial 3 shows how to draw
rectangles, circles, lines, etc." width=200px \include
"../apps/tutorials/tuto3_Drawing.cpp"

<br>
<br>
<br>
\subsection tuto4 Tutorial 4: Keyboard
\image html tuto_drawing.jpg "The code of the tutorial 4 shows how to manage the
keyboard." width=200px \include "../apps/tutorials/tuto4_Keyboard.cpp"

<br>
<br>
<br>
\subsection tuto5 Tutorial 5: Mouse
\image html tuto_mouse.jpg "The code of the tutorial 5 shows how manage the
mouse to draw a square" width=200px \include "../apps/tutorials/tuto5_Mouse.cpp"

<br>
<br>
<br>
\subsection tuto6 Tutorial 6: Menu
\image html tuto_menu.jpg "The code of the tutorial 6 shows how to create a
simple menu." width=200px \include "../apps/tutorials/tuto6_Menu.cpp"

<br>
<br>
<br>
\subsection tuto7 Tutorial 7: Animation
\image html tuto_animation.jpg "The code of the tutorial 2 shows how to display
a simple animation depending on the elapsed time." width=200px \include
"../apps/tutorials/tuto7_Animation.cpp"

<br>
<br>
<br>
\subsection tuto8 Tutorial 8: Plot/Graph
\image html tuto_plot.jpg "The plot code illustrates the drawing of a graph
entering values of a function" width=200px \include
"../apps/tutorials/tuto8_Plot.cpp"

<br>
<br>
<br>
\subsection tuto9 Tutorial 9: Image
\image html tuto_image.jpg "The demo code illustrates image functionalities of
Grapic" width=200px \include "../apps/tutorials/tuto9_Image.cpp"
...
\image html tuto_demo.jpg "The demo code illustrates many functionalities of
Grapic" width=200px \include "../apps/tutorials/tuto10_Demo.cpp"
...
Grapic comes with several small examples. It can be used as source of
inspiration.
...
You can check and ask your question in the issues section of GitHub :
[https://github.com/ucacaxm/grapic/issues](https://github.com/ucacaxm/grapic/issues)


- int main() ==> int main(int , char **)

- Image im = image("data\papillon.jpg") ==> Image im =
image("ProgramData//papillon.jpg")   or Image im = image("data\\papillon.jpg")

- Even tutorials do not compile
   - check in directory 'extern' that you have the version for your computer:
mingw for windows, macox for mac and nothing for linux since you have to install
them with the apt explained in the Section installtion
   - Try to rerun premake with premake.bat on windows or make premake on
linux/macos
